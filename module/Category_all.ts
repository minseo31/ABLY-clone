...
// 카테고리 사이드 바
const sidebar = document.querySelector(".category-sidebar") as HTMLDivElement;
// 카테고리 종류 컨테이너
const itemBox = document.querySelector(".category-itemBox") as HTMLDivElement;
let listTop : number[] = []; 

// 사이드 바 카테고리 아이템 생성
datas.map((data, i) => {
  const listElement = document.createElement("span") as HTMLSpanElement;
  ...
  listElement.addEventListener("click", () => {
    datas.forEach((d, index) => {
      const list = document.querySelector(
        `.category-sidebar-list:nth-child(${index + 1})`
      ) as HTMLDivElement;
    //   클릭한 엘리먼트만 변경
      if (index === i) {
  ...
  //카테고리 타이틀
  const categoryTilte = document.createElement("span") as HTMLSpanElement;
  categoryTilte.classList.add("category-title");
  categoryTilte.innerText = `${data.item.name}`;

  //   카테고리 박스
  const categoryBox = document.createElement("div") as HTMLDivElement;
  categoryBox.classList.add("category-box");
  categoryBox.appendChild(categoryTilte);

  //   카테고리 그리드 아이템 박스
  const itemElementBox = document.createElement("div") as HTMLDivElement;
  itemElementBox.classList.add("items-box");
  itemElementBox.style.gridTemplateRows = `repeat(${
    data.item.subCategoryList.length / 3
  }, minmax(0, 1fr))`;
  ...
  data.item.subCategoryList.map((subData, i) => {
    ...
    const imgBox = document.createElement("div") as HTMLDivElement;
    imgBox.classList.add("category-img-Box");

    // 아이템 이미지
    const imgItemElement = document.createElement("img") as HTMLImageElement;
    imgItemElement.src = `${subData.item.image}`;
    imgItemElement.classList.add("item-img");

    // 아이템 이름
    const itemNameElement = document.createElement("span") as HTMLSpanElement;
...
// 전체보기 컨테이너
const categoryOverView = document.querySelector(
  ".category-all"
) as HTMLElementTagNameMap["section"];
// 전체보기 이동 아이콘
const n2 = document.getElementById("n2") as HTMLDivElement;
// 뒤로가기 버튼
const back = document.getElementById("category-back") as HTMLElement;
